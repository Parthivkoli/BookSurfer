...
export default function ReaderClient({ initialBook, initialContent, bookId }: ReaderClientProps) {
  ...
  useEffect(() => {
    const paginateContent = () => {
      ...
      for (const paragraph of paragraphs) {
        ...
        tempDiv.innerHTML = `<div>${testContent.split("\n\n").map(p => `<p>${p}</p>`).join("")}</div>`;
        ...
        if (tempDiv.offsetHeight <= containerHeight) {
          ...
          if (currentPageContent) {
            ...
            for (const word of words) {
              const testPartial = partialParagraph ? `${partialParagraph} ${word}` : word;
      ...
      const totalWords = pages.reduce((total, page) => total + page.split(/\s+/).length, 0);
      ...
      setEstimatedTimeLeft(minutesLeft < 60 ? `~${minutesLeft} min left` : `~${Math.floor(minutesLeft / 60)}h ${minutesLeft % 60}m left`);
    };
  ...
  useEffect(() => {
    const loadSavedState = () => {
      try {
        ...
        if (savedProgress) setCurrentPage(Math.min(parseInt(savedProgress, 10), contentPages.length - 1) || 0);
  ...
  const handlePageJump = () => {
    ...
    if (pageNum >= 0 && pageNum < contentPages.length) {
      ...
      setPageInput("");
      if (isPlaying) stopTextToSpeech();
      toast({ title: "Page Changed", description: `Moved to page ${pageNum + 1}`, duration: 1500 });
    } else {
      toast({ title: "Invalid Page", description: `Enter a number between 1 and ${contentPages.length}`, variant: "destructive", duration: 3000 });
    }
  };
  ...
  const toggleBookmark = () => {
    ...
    if (index === -1) {
      ...
      toast({ title: "Bookmark Added", description: `Page ${currentPage + 1} bookmarked`, duration: 2000 });
    } else {
      newBookmarks.splice(index, 1);
      toast({ title: "Bookmark Removed", description: `Bookmark for page ${currentPage + 1} removed`, duration: 2000 });
    }
  ...
  // **AI Summary**
  const summarizeContent = (content, maxSentences = 3) => {
  ...
  const generateAnswer = (content, question, maxSentences = 2) => {
    const lowerContent = content.toLowerCase();
    const lowerQuestion = question.toLowerCase().trim();
    
    const questionWords = lowerQuestion.split(/\s+/)
      .filter(word => word.length > 3 && !['what', 'is', 'are', 'the', 'a', 'an', 'of'].includes(word));
    ...
    if (bestMatch) {
      const intent = lowerQuestion.startsWith("what") ? "Here's what I found: " :
                    lowerQuestion.startsWith("who") ? "This is who: " :
                    lowerQuestion.startsWith("why") ? "Here's why: " : "Here's the answer: ";
      
      const responseSentences = bestMatch.split(/[.!?]+/).filter(s => s.trim()).slice(0, maxSentences);
      return `${intent}${responseSentences.join(". ")}.`;
    }

    return `I couldn’t find a direct answer, but here’s some related information from the page: ${sentences.slice(0, maxSentences).join(". ")}.`;
  };
  ...
  const startTextToSpeech = () => {
    ...
    utterance.onboundary = (event) => {
      if (event.name === "word" && event.charIndex !== undefined) {
        ...
        let wordIndex = wordStarts.findIndex((start, i) => 
          start <= charIndex && (i === wordStarts.length - 1 || wordStarts[i + 1] > charIndex)
        );
        if (wordIndex >= 0) {
    ...
    utterance.onend = () => {
      ...
      if (currentPage < contentPages.length - 1) {
        toast({ title: "Reading Complete", description: "Moving to next page", duration: 2000 });
        setTimeout(goToNextPage, 1500);
      } else {
        toast({ title: "Reading Complete", description: "End of content reached", duration: 2000 });
      }
    };
    ...
    setIsPlaying(true);
    toast({ title: "Reading Started", description: selectedVoice?.name || "Text-to-speech started", duration: 1500 });
  };
  ...
  const pauseTextToSpeech = () => {
    if (isPlaying) {
      ...
      setIsPlaying(false);
      toast({ title: "Reading Paused", description: "Text-to-speech paused", duration: 1500 });
    }
  };
  ...
  const resumeTextToSpeech = () => {
    if (!isPlaying && utteranceRef.current) {
      ...
      setIsPlaying(true);
      toast({ title: "Reading Resumed", description: "Text-to-speech resumed", duration: 1500 });
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      else if (e.key === "Escape" && isFullScreen) document.exitFullscreen().catch(console.error);
      else if (e.key === "p" || e.key === " ") {
        e.preventDefault();
        isPlaying ? pauseTextToSpeech() : (utteranceRef.current ? resumeTextToSpeech() : startTextToSpeech());
  ...
  const getWordDefinition = async (word: string, e: React.MouseEvent<HTMLSpanElement>) => {
    ...
    const left = e.clientX + offsetX < window.innerWidth - panelWidth ? e.clientX + offsetX : e.clientX - panelWidth - offsetX;
    ...
    try {
      ...
      if (data && data.length > 0 && data[0].meanings && data[0].meanings.length > 0) {
        ...
        const definition = meaning.definitions[0].definition || "No definition available for this meaning.";
        const example = meaning.definitions[0].example ? `\nExample: ${meaning.definitions[0].example}` : "";
        setWordDefinition(`${definition}${example}`);
      } else {
        setWordDefinition(`No definition found for "${cleanWord}". It might be a proper noun, slang, or not in our dictionary.`);
      }
      setShowDefinitionPanel(true);
    } catch (error) {
      console.error(`Error fetching definition for "${cleanWord}":`, error);
      setWordDefinition(`Could not fetch definition for "${cleanWord}". Please check your network or try again later.`);
      setShowDefinitionPanel(true);
    }
  };
  ...
  const getMarginClass = () => {
    switch (marginSize) {
      case "small": return "px-2 sm:px-4";
      case "large": return "px-12 md:px-24";
      default: return "px-8 md:px-16";
    }
  };
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-center">
          ...
          >
            ...
            <p className="text-muted-foreground mb-8">We couldn't find the book you're looking for.</p>
            <Button onClick={() => router.push("/discover")} size="lg" className="w-full">
      ...
      <div className={`min-h-screen flex flex-col ${isFullScreen ? "bg-gray-100 dark:bg-gray-900" : ""}`}>
        ...
        {!isFullScreen && (
          <div className="flex justify-center items-center p-4 bg-white dark:bg-gray-800 shadow-md">
            ...
            <h1 className="text-xl font-bold mx-auto text-center">{book.title}</h1>
        ...
        <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
          <SheetContent side="left">
            ...
            <div className="py-6 space-y-4">
              <Button variant="ghost" onClick={() => router.push("/discover")}>Discover</Button>
              <Button variant="ghost" onClick={() => router.push("/library")}>My Library</Button>
              <Button variant="ghost" onClick={() => router.push("/reader")}>Reader</Button>
        ...
        <div
          ref={mainContentRef}
          className={`flex-1 flex flex-col items-center ${isFullScreen ? "p-0" : "py-4 sm:py-8"}`}
        >
          <div className={`w-full max-w-4xl mx-auto ${isFullScreen ? "h-screen" : "min-h-[70vh]"} ${getMarginClass()}`}>
            <div className="w-full bg-gray-200 dark:bg-gray-800 h-1 mb-4">
              <div className="h-full bg-primary transition-all" style={{ width: `${bookProgress}%` }} />
            </div>
            <motion.div
            ...
            >
              <div
                ...
                style={{ fontSize: `clamp(14px, 4vw, ${fontSize}px)`, lineHeight: lineHeight }}
              >
            ...
            <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <Button variant="outline" size="icon" className="h-12 w-12" onClick={goToPrevPage} disabled={currentPage <= 0}>
              ...
              />
              <Button variant="outline" size="icon" className="h-12 w-12" onClick={goToNextPage} disabled={currentPage >= contentPages.length - 1}>
              ...
              <div className="space-y-1">
                {bookmarks.map((bookmark) => (
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className={`w-full justify-start ${currentPage === bookmark ? "bg-primary/10 text-primary" : ""}`}
        ...
        {/* AI Assistant Panel */}
        <AnimatePresence>
          {isFullScreen && aiPanelOpen && controlsVisible && (
            <motion.div
              initial={{ opacity: 0, x: 100 }}
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: 100 }}
            ...
            >
              ...
              <Tabs defaultValue="summary" className="flex-1 overflow-hidden flex flex-col">
                ...
                <TabsContent value="summary" className="flex-1 overflow-y-auto p-3 space-y-3">
                  <p className="text-sm text-muted-foreground">Get an AI-generated summary of the current page</p>
                  {summaryLoading ? (
                      ...
                      <CardFooter className="p-2 flex justify-end">
                        <Button 
                          ...
                          onClick={() => {
                            ...
                            toast({ title: "Copied", description: "Summary copied to clipboard", duration: 1500 });
                          }}
                        >
                ...
                <TabsContent value="ask" className="flex-1 overflow-hidden flex flex-col">
                  <div className="flex-1 overflow-y-auto p-3 space-y-3">
                    <p className="text-sm text-muted-foreground">Ask a question about the current page (e.g., "What are the main themes?")</p>
                    <Textarea
                        ...
                        <CardFooter className="p-2 flex justify-end space-x-2">
                          <Button 
                            ...
                            onClick={() => {
                              ...
                              toast({ title: "Copied", description: "Answer copied to clipboard", duration: 1500 });
                            }}
                          >
                            <Copy className="h-4 w-4 mr-1" /> Copy
                          </Button>
