...
async function fetchCorePapers(
  ...
  try {
    ...
    coreData.results.slice(0, maxResults).forEach((result: any) => {
...
async function fetchSemanticScholarPapers(
  ...
  try {
    ...
    const semanticUrl = `https://api.semanticscholar.org/graph/v1/paper/search?query=${encodeURIComponent(
      query
    ...
    semanticData.data.slice(0, maxResults).forEach((paper: any) => {
      papers.push({
...
async function fetchOpenAirePapers(
  ...
  try {
    const openAireUrl = `https://api.openaire.eu/search/publications?format=json&size=${maxResults}&title=${encodeURIComponent(
      query
    )}`;
    ...
    results.slice(0, maxResults).forEach((result: any) => {
...
async function fetchPubMedPapers(
  ...
  try {
    ...
    const efetchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id=${ids.join(
      ","
    )}&retmode=xml`;
...
async function fetchDoajPapers(
  ...
  try {
    const doajUrl = `https://doaj.org/api/search/articles/${encodeURIComponent(
      query
    )}?pageSize=${maxResults}`;
